...
CI retains the original workbench, OCR/color, tiled OCR, IMAGE, appearance, native CLI and independent DXF checks. Version 0.6 adds worker/reference-cache browser tests and source tests for transaction isolation, numerical memoization, cancellation, deterministic BVH ordering and browser import-map resolution. The source acceptance record is in [VALIDATION-0.6.md](VALIDATION-0.6.md).

After deployment, six browser suites navigate to the public HTTPS application: base deployment, OCR/colors, tiled recovery, native images, sampled appearance and conversion performance behavior. The base suite checks the exact deployed commit. The new suite verifies actual worker reuse, PDF-reference cache hits/invalidation, world-coordinate rescaling, allocation release and hard cancellation/recovery. No fabricated conversion results or local inline harness substitute for this public-site gate.
...
`performance.yml` is a read-only workflow that compares the checked-out source with the isolated 0.5.0 baseline at `3b3bf2101c92da8f50773fe62f14c092cf31b935`. It installs separate workspace dependency links, performs 746 differential checks and runs thirteen unprofiled before/after workloads on the same runner. It does not invoke OCR, alter source, publish packages or reduce conversion quality.

The `conversion-performance` artifact binds measured samples and correctness hashes to a commit. Ratios are observations rather than timing-based pass/fail limits; output equivalence is mandatory. Future intended semantic/output changes require explicit review of the historical baseline. The checked-in v0.6 acceptance includes separate V8 profiles that did not instrument headline benchmark timings. See [PERFORMANCE.md](PERFORMANCE.md) for methodology, scope and reproducible commands.

## npm distribution and registry status

Sixteen version 0.6.0 package archives, exact dependency metadata and integrity manifests are tracked in `release/npm/`. Source acceptance installs and imports them in a clean offline consumer. CI also validates publication through a dry run. Neither step establishes registry publication.
